# for creating the template we feed to llm
from langchain.prompts import PromptTemplate
from langchain_community.chat_models import QianfanChatEndpoint
from langchain.embeddings import QianfanEmbeddingsEndpoint
# ^ for getting the actual GPT llm and also the embeddor
# for converting llm output to something Python understands
...
# for making HTTP request in Python to handle headers, cookies, and authentication
import requests
# for parsing HTML and XML docs so we can get j the urls / body text directly from a webpage
...
def split_text(text):
    ...
    for word in words:  # for each word
        # if adding this word doesn't exceed 400 words
        if len(" ".join(current_chunk + [word])) <= 400 and word:
            current_chunk.append(word)  # then it is added
        elif current_chunk:
            # else the current chunk is added to the chunks list as a single string
            chunks.append(" ".join(current_chunk))
            # then current_chunk is refreshed into a new chunk
            current_chunk = [word]

    if current_chunk:
        # then at the end if there's a current chunk left then that's added to the list
        chunks.append(" ".join(current_chunk))
...
def baidu_search(query):  # query being the context we got
    ...
    # wd stands for word/query which is what we're searching (the context we're taking in)
    ...
    for item in soup.find_all('div', class_='result'):
        ...
        if link:
            # and if it's a link then we add it into the list w the 'href' attribute which is a url
    ...
    for doc in docs:  # for each doc in the docs list
        # this regex part is p cool, sub() obviously substitutes something w another thing
        page_text = re.sub("\n\n+", "\n", doc)
...
def get_page(urls):
    ...
    for url in urls:
        ...
        # this time we r j popping into all the pages
        ...
        if response.status_code == 200:  # status code 200 means that it was successful so if we were let in, then we call beautiful soup
            ...
            # we initiate the soup object again
            ...
            # 'p' = praragraph so now we r making a list of all the paragraphs from the parsed html
            paragraphs = soup.find_all('p')
            # ^ the issue is what if there are text content thats important thats not marked as 'p' or what if 'p' isn't right?
            # and we extract all the text from the paragraphs & join them
            page_text = "\n".join([p.get_text() for p in paragraphs])
            # so that it doesn't keep causing errors
            chunks = split_text(page_text)
            # and now we add the text into our list of page content
            docs.extend(chunks)
...
def rag_search(query):
    # rn we hv them as diff functions in case baidu doesn't work out
...
# this parses the output param that is generated by the llm to find the info we need for the df
def parse_response(llm_output):
    ...
    # this is for debugging in case we find something isn't generated properly
    product_description = 'missing description'
    # then we know that whatever has "missing description" didn't have that part
    ...
    for line in lines:  # for each loop that searches each line in the output for the content we need
        # so if the line starts with [this] then we store this line into the corresponding var
        if line.startswith("1. 产品描述："):
            # but we strip the title for cleanliness
            product_description = line[8:].strip()
    ...
    # and we just return the variables
...
def call_marketinGPT():
    ...
    # 'r' means read (DUH), utf-8 encoding is standard
    ...
    # prod_descr prior to this would be like ["prod_des1\n", "prod_des2\n"]
    ...
    prompt1 = PromptTemplate(
        ...
        # here ur telling the gpt that the input variables it uses will be
    ...
    # here ur declaring the arrays that the df will be made w
    ar1, ar2, ar3, ar4 = [], [], [], []
    ...
    for prod_des in prod_descr:  # this is a for-each loop which ensures that each val in the list is used
        ...
        # uses the langchain qianfan embedder
        ctxt_embed = embed.embed_query(ctxt)
        # calls the RAG search func to get the list of search results for the context we r looking for
        ...
        for result in search_results:  # for every single result we get
            # we split the result into a list of chunks
            chunks = split_text(result)
            # and adds all the chunks to the list separately
        ...
        while i2 < len(all_chunks) - 1:  # while loop for until all chunks r processed
            i2 += 1
            # then embeds the search results chunk by chunk
        ...
        # turns the top 3 results into one string to be fed into prompt
        rag_results = " ".join([result[0] for result in top_results])

        # this is the setup for the processing pipeline
        ...
        try:  # we using a try-except bc who knows if the GPT will output something that is always understandable
            ...
            # marketinGPT (brilliant name) is the name of the pipeline
            ...
            # here we use parse_response to parse the response (DUH)
            parsed_response = parse_response(ans)

            # here we r appending (adding) the answer to the arrays
            ar1.append(parsed_response[0])
            ar2.append(parsed_response[1])  # hopefully everything is right
            # but otherwise it will all be missing descriptions
            ar3.append(parsed_response[2])
            # so this tells us if the GPT failed to generate an expected portion
            ar4.append(parsed_response[3])

        except Exception as e:  # this is just the exception portion
            # we r just saying if we couldn't process any part of the inputs
            print(f"Couldn't process: {prod_des}. Error: {e}")
            ar1.append('could not process')
            ar2.append('could not process')
            ar3.append('could not process')
            ar4.append('could not process')

    # finally we make the df w the arrays we built
    return df_mk(ar1, ar2, ar3, ar4)
...
styled_df = df.style.set_properties(**{  # so we're just making another df based on the generated df but w styling
    # the syntax for setting stuff is [what u wanna set]: [val]
    'background-color': 'aliceblue',
    'color': 'black',
    'text-align': 'center',
    'border': '2px solid lightsteelblue !important'
    # below, the selector means it applies to all the following, being thead, which is the table header section (thead)'s table headers (th)
}).set_table_styles([
    {'selector': 'thead th', 'props': [('background-color', 'lightslategrey'), ('color', 'white'),
                                       ('border', '2px solid darkslategray !important'), ('text-align', 'center')]}
